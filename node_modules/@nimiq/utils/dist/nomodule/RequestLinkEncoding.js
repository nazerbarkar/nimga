...
const DECIMALS = {
    [exports.Currency.NIM]: 5,
    [exports.Currency.BTC]: 8,
    [exports.Currency.ETH]: 18,
    [exports.Currency.MATIC]: 18,
    [exports.Currency.USDC]: 6,
    [exports.Currency.USDT]: 6,
};
...
})(exports.EthereumChain || (exports.EthereumChain = {}));
...
const ETHEREUM_SUPPORTED_NATIVE_CURRENCIES = [exports.Currency.ETH, exports.Currency.MATIC];
const ETHEREUM_SUPPORTED_CONTRACTS = {
    [exports.EthereumChain.ETHEREUM_MAINNET]: {
        [exports.Currency.USDC]: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',
        [exports.Currency.USDT]: '0xdac17f958d2ee523a2206206994597c13d831ec7',
    },
    [exports.EthereumChain.ETHEREUM_SEPOLIA_TESTNET]: {
        [exports.Currency.USDC]: '0xf08a50178dfcde18524640ea6618a1f965821715',
        [exports.Currency.USDT]: '0xaa8e23fb1079ea71e0a56f48a2aa51851d8433d0',
    },
    [exports.EthereumChain.POLYGON_MAINNET]: {
        [exports.Currency.USDC]: '0x3c499c542cef5e3811e1192ce70d8cc03d5c3359',
        [exports.Currency.USDT]: '0xc2132d05d31c914a87c6611c10748aeb04b58e8f',
    },
    [exports.EthereumChain.POLYGON_AMOY_TESTNET]: {
        [exports.Currency.USDC]: '0x41e94eb019c0762f9bfcf9fb1e58725bfb0e7582',
        [exports.Currency.USDT]: '0x1616d425cd540b256475cbfb604586c8598ec0fb',
    },
};
...
const ETHEREUM_PATH_REGEX = new RegExp('^'
    ...
    + '$');
exports.NimiqRequestLinkType = void 0;
...
})(exports.NimiqRequestLinkType || (exports.NimiqRequestLinkType = {}));
...
message, basePath = window.location.host) {
    if (typeof amountOrOptions === 'object') {
        switch (amountOrOptions.currency) {
            case exports.Currency.NIM:
                return createNimiqRequestLink(recipient, amountOrOptions);
            case exports.Currency.BTC:
                return createBitcoinRequestLink(recipient, amountOrOptions);
            case exports.Currency.ETH:
            case exports.Currency.MATIC:
            case exports.Currency.USDC:
            case exports.Currency.USDT:
    ...
    const amount = typeof amountOrOptions !== 'undefined'
        ? amountOrOptions * (10 ** DECIMALS[exports.Currency.NIM])
        : undefined;
    return createNimiqRequestLink(recipient, { amount, message, basePath });
}
function parseRequestLink(requestLink, options = {}) {
    const currencies = options.currencies || Object.values(exports.Currency);
    ...
    if (currencies.includes(exports.Currency.NIM) && /^(web\+)?nim(iq)?:$/i.test(url.protocol)) {
        return addCurrencyToResult(parseNimiqUriRequestLink(url), exports.Currency.NIM);
    }
    if (currencies.includes(exports.Currency.NIM) && /^https?:$/i.test(url.protocol)) {
        return addCurrencyToResult(parseNimiqSafeRequestLink(url, expectedNimiqSafeRequestLinkBasePath), exports.Currency.NIM);
    }
    if (currencies.includes(exports.Currency.BTC) && /^bitcoin:$/i.test(url.protocol)) {
        return addCurrencyToResult(parseBitcoinRequestLink(url, isValidAddress[exports.Currency.BTC], normalizeAddress[exports.Currency.BTC]), exports.Currency.BTC);
    }
    if ([...ETHEREUM_SUPPORTED_NATIVE_CURRENCIES, exports.Currency.USDC, exports.Currency.USDT]
        .some((currency) => currencies.includes(currency))
        && new RegExp(`^(${Object.values(EthereumBlockchainName).join('|')}):$`, 'i').test(url.protocol)) {
        const parsedRequestLink = parseEthereumRequestLink(url, isValidAddress[exports.Currency.ETH], normalizeAddress[exports.Currency.ETH]);
...
function createNimiqRequestLink(recipient, options = { basePath: window.location.host }) {
    const { amount, message, label, basePath, type = exports.NimiqRequestLinkType.SAFE } = options;
    if (!ValidationUtils.ValidationUtils.isValidAddress(recipient))
    ...
    if (message && (typeof message !== 'string'
        ...
        || Utf8Tools.Utf8Tools.stringToUtf8ByteArray(message).byteLength > 64))
        throw new Error(`Not a valid message: ${message}`);
    if (label && typeof label !== 'string')
        throw new Error(`Not a valid label: ${label}`);
    recipient = ValidationUtils.ValidationUtils.normalizeAddress(recipient).replace(/ /g, ''); // normalize and strip spaces
    const amountNim = amount
        ? new FormattableNumber.FormattableNumber(amount).moveDecimalSeparator(-DECIMALS[exports.Currency.NIM]).toString()
        : '';
    // Assemble params
    ...
    if (type === exports.NimiqRequestLinkType.SAFE) {
    ...
    if (type === exports.NimiqRequestLinkType.URI || type === exports.NimiqRequestLinkType.WEBURI) {
...
function parseNimiqParams(params) {
    const recipient = ValidationUtils.ValidationUtils.normalizeAddress(params.recipient);
    if (!ValidationUtils.ValidationUtils.isValidAddress(recipient))
        return null; // recipient is required
    const amount = params.amount ? Math.round(parseFloat(params.amount) * (10 ** DECIMALS[exports.Currency.NIM])) : undefined;
    if (typeof amount === 'number' && !isUnsignedSafeInteger(amount))
        return null;
    const { label, message } = params;
    // Message length is limited to 64 bytes, see BasicAccount.verifyIncomingTransaction in Nimiq core.
    if (message && Utf8Tools.Utf8Tools.stringToUtf8ByteArray(message).byteLength > 64)
        return null;
...
options = {}) {
    ...
    validQueryKeys.forEach((key) => {
        ...
        const formattedValue = key === 'amount' || key === 'fee'
            ? new FormattableNumber.FormattableNumber(option).moveDecimalSeparator(-DECIMALS[exports.Currency.BTC]).toString()
...
function parseBitcoinRequestLink(requestLink, isValidAddress, normalizeAddress) {
    ...
    if (!recipient || (isValidAddress && !isValidAddress(recipient)))
        return null; // recipient is required
    const amount = rawAmount ? Math.round(parseFloat(rawAmount) * (10 ** DECIMALS[exports.Currency.BTC])) : undefined;
    if (typeof amount === 'number' && !isUnsignedSafeInteger(amount))
        return null;
    const fee = rawFee ? Math.round(parseFloat(rawFee) * (10 ** DECIMALS[exports.Currency.BTC])) : undefined;
...
currency, options) {
    ...
    const chainIdString = chainId !== undefined && chainId !== exports.EthereumChain.ETHEREUM_MAINNET ? `@${chainId}` : '';
    ...
    if (amount) {
        ...
        const formattableNumber = new FormattableNumber.FormattableNumber(amount);
    ...
    if (gasPrice) {
        ...
        const formattableNumber = new FormattableNumber.FormattableNumber(gasPrice);
        formattableNumber.moveDecimalSeparator(-9);
        query.set('gasPrice', `${formattableNumber.toString()}e9`);
    }
    if (gasLimit) {
        query.set('gasLimit', FormattableNumber.toNonScientificNumberString(gasLimit));
    }
...
function parseEthereumRequestLink(requestLink, isValidAddress = validateEthereumAddress, normalizeAddress = (address) => address) {
    ...
    const chainId = rawChainId ? parseInt(rawChainId, 10) : (contractChainId
        ...
        || (url.protocol === `${EthereumBlockchainName.POLYGON}:` ? exports.EthereumChain.POLYGON_MAINNET : undefined));
    if (typeof chainId === 'number' && (!isUnsignedSafeInteger(chainId)
        || !Object.values(exports.EthereumChain).includes(chainId)
...
function parseUnsignedInteger(value) {
    value = FormattableNumber.toNonScientificNumberString(value); // Resolves scientific notation and throws for invalid number strings.
...
function getEthereumBlockchainName(chainIdOrNativeCurrency) {
    switch (chainIdOrNativeCurrency) {
        case exports.EthereumChain.ETHEREUM_MAINNET:
        case exports.EthereumChain.ETHEREUM_SEPOLIA_TESTNET:
        case exports.Currency.ETH:
            return EthereumBlockchainName.ETHEREUM;
        case exports.EthereumChain.POLYGON_MAINNET:
        case exports.EthereumChain.POLYGON_AMOY_TESTNET:
        case exports.Currency.MATIC:
...
function getEthereumCurrency(chainIdOrBlockchainName) {
    switch (chainIdOrBlockchainName) {
        ...
        case EthereumBlockchainName.ETHEREUM:
            return exports.Currency.ETH;
        case exports.EthereumChain.POLYGON_MAINNET:
        case exports.EthereumChain.POLYGON_AMOY_TESTNET:
        case EthereumBlockchainName.POLYGON:
            return exports.Currency.MATIC;
...
exports.ETHEREUM_SUPPORTED_CONTRACTS = ETHEREUM_SUPPORTED_CONTRACTS;
exports.ETHEREUM_SUPPORTED_CONTRACTS_REVERSE_LOOKUP = ETHEREUM_SUPPORTED_CONTRACTS_REVERSE_LOOKUP;
exports.ETHEREUM_SUPPORTED_NATIVE_CURRENCIES = ETHEREUM_SUPPORTED_NATIVE_CURRENCIES;
exports.createBitcoinRequestLink = createBitcoinRequestLink;
exports.createEthereumRequestLink = createEthereumRequestLink;
exports.createNimiqRequestLink = createNimiqRequestLink;
exports.createRequestLink = createRequestLink;
exports.parseBitcoinRequestLink = parseBitcoinRequestLink;
exports.parseEthereumRequestLink = parseEthereumRequestLink;
exports.parseNimiqSafeRequestLink = parseNimiqSafeRequestLink;
exports.parseNimiqUriRequestLink = parseNimiqUriRequestLink;
exports.parseRequestLink = parseRequestLink;
